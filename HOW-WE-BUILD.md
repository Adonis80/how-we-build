...
5. The named reviewer reads the PR cold in its own session, and no other stack reads for the gate (his ruling, 22 September). Asked once per round, fixes batched into one push; with no read the gate can count, the slice parks and the next begins. The gate never opens unreviewed.
