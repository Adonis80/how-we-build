...
**The loop.** One builder session per slice, attached to the repo. A turn stays only while the work is the same, staying is cheaper than a fresh start from GitHub, and the history still helps; otherwise it checkpoints and starts fresh. When nothing can move it stops hard: no clock, timer, schedule or automation wakes it. Words change from Cowork by pull request.
