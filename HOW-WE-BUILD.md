...
**The unit of work** is one accepted slice: a user-visible outcome with acceptance criteria, never a screen. **"Build" is a whole instruction**: take the top `roadmap.json` item carrying his word and build it, settling everything technical yourself. It never means run the build, and is never answered with a question.

**The loop.** One builder session per slice: a code session attached to the repo; words change from Cowork by pull request, kept while what it holds stays true.
