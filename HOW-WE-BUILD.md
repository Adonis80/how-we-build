...
**Who decides.** Dhayan is Chairman: the customer problem, what the product does, business figures and pricing rules, material visual acceptance, spending, permissions, anything irreversible. Claude is CTO: every other technical decision; never ask him to choose between technical options. When unsure, take the smallest reversible option, record it in the PR, continue. A turn never ends on a question the CTO could answer.
...
**The loop.** Fresh builder session per slice; chat sessions never edit the repo meanwhile.
...
**Every product has** one private repo (`AGENTS.md`: first line points here, then only what is true there, under 500 words, enforced by `check.sh`; `PRODUCT.md`; `NAMES.md`; `roadmap.json`; `README.md`) and one Claude Project, holding no files: a few lines pointing at both, how a session gets in when the cloud cannot, and the Chairman's prose. Nothing for the Chairman goes into a repo.
