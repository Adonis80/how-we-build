...
GITHUB_ENTRIES = {'workflows': 'dir'}
WORKFLOW_FILES = {'check.yml'}
...
def repo_errors(root):
    ...
    github = root_items.get('.github')
    if github and github.is_dir() and not github.is_symlink():
        try:
            github_items = {p.name: p for p in github.iterdir()}
        except OSError:
            github_items = {}
            errors.append('GitHub configuration directory cannot be read')
        if set(github_items) != set(GITHUB_ENTRIES):
            errors.append('required GitHub configuration entries are missing or unexpected entries exist')
        workflows = github_items.get('workflows')
        if workflows is not None:
            if workflows.is_symlink() or not workflows.is_dir():
                errors.append('a GitHub configuration entry has the wrong type')
            else:
                try:
                    workflow_items = {p.name: p for p in workflows.iterdir()}
                except OSError:
                    workflow_items = {}
                    errors.append('workflow directory cannot be read')
                if set(workflow_items) != WORKFLOW_FILES:
                    errors.append('required workflow entries are missing or unexpected entries exist')
                if any(p.is_symlink() or not p.is_file() for p in workflow_items.values()):
                    errors.append('a workflow entry has the wrong type')
...
def reviewed_head(head, reviews, comments, resolve):
    ...
    for review in head_reviews:
        if (review.get('submitted_at') and
                review.get('state') in {'APPROVED', 'CHANGES_REQUESTED', 'COMMENTED'}):
            return True
    # A clean-pass comment is a fallback only when no review record exists for
    # this head. A pending or dismissed record must not be rescued by the bot's
    # separate, persistent summary comment.
    if head_reviews:
        return False
...
def check_review():
    ...
    try:
        ...
        if reviewed_head(head, reviews, comments, lambda ref: api.get(f'/commits/{ref}')['sha']):
            return None
        return 'no completed Codex review receipt exists for the current head'
...
def fixture():
    ...
    (root / '.github' / 'workflows' / 'check.yml').write_text('safe', encoding='utf-8')
    (root / 'design').mkdir()
    for name, kind in ROOT_ENTRIES.items():
        path = root / name
        if kind == 'file':
...
class GuardRegressions(unittest.TestCase):
    ...
    def test_workflow_rechecks_clean_pass_comments(self):
        workflow = (ROOT / '.github' / 'workflows' / 'check.yml').read_text(encoding='utf-8')
        self.assertRegex(workflow, r'(?m)^  issue_comment:\s*$')
        self.assertIn("github.event.comment.user.login == 'chatgpt-codex-connector[bot]'", workflow)
    ...
    def test_dismissed_review_blocks_persistent_summary_comment(self):
        head = 'a' * 40
        dismissed = {
            'user': {'login': BOT}, 'commit_id': head,
            'submitted_at': 'now', 'state': 'DISMISSED',
        }
        summary = {
            'user': {'login': BOT},
            'body': (SUMMARY_MARKER + '\n'
                     ...
                     '| --- | --- | --- | --- |\n'
                     f'| 📝 **Code Review** | ✅ **Completed** now | `{head[:7]}` | Manual request |'),
        }
        self.assertFalse(reviewed_head(head, [dismissed], [summary], lambda _: head))
...
if os.environ.get('GITHUB_EVENT_NAME') in {'pull_request', 'pull_request_review', 'issue_comment'}:
    review_error = check_review()
    if review_error:
        print('FAIL: ' + review_error)
        errors.append(review_error)
    else:
        print('ok: completed Codex review receipt verified for current head')
if not errors:
