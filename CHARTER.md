...
Which model leads is the Chairman's, and changes as the models do; because the rules are written by role, that swap costs nothing — the lead is a role no machine here runs. A model a machine runs, such as the reviewer, is another matter, and `library/review-machinery.md`, *A model switched in one edit*, says what a switch costs today. What each stack can actually do, and how its workspace is configured, lives in `library/two-stacks.md`, which changes faster than this Charter.
