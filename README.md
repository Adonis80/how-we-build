...
**Cold, and read twice where it counts.** Independent means a session of its own, forming its view from the diff and the tests before reading the pull request's own account. Two reviewers read every pull request — different models, each cold, each signing a verdict of its own. On pricing logic, live database mutation or schema, authentication and authorisation, public trust-boundary changes, deploy and release machinery, and this review gate itself, **both** must read it clean before the gate opens. Everywhere else one clean read is enough. If it is unclear whether a change crosses a trust boundary, it does.

**That rule is the intent and is not yet in force, and the difference matters most on the one class a machine can see.** `review-gate.py` classifies gate changes by file, and for those it opens on `GATE_REVIEWER` alone — deliberately, so that findings from a reviewer with no standing on a gate change cannot bolt a door it may not open (round #38, accepted in part). "Both must be clean" is the opposite of that, and turning it on is a change to the clearing logic and a selftest case, not a sentence here. The other five classes no file pattern classifies at all; they were a practice the CTO kept before this ruling and they are one after it. So: one reviewer's clean read still clears a gate change today, and until the logic changes this page is describing where it is going. Every pull request carries `Lead stack:` and `Reviewed by:`, so the pair is visible without asking.
