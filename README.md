...
**Cold, and cross-vendor where it counts.** Independent means a session of its own, forming its view from the diff and the tests before reading the pull request's own account. The same vendor is acceptable; the other vendor is preferred; and on pricing logic, live database mutation or schema, authentication and authorisation, public trust-boundary changes, deploy and release machinery, and this review gate itself, the other vendor is required. If it is unclear whether a change crosses a trust boundary, it does. Every pull request carries `Lead stack:` and `Reviewed by:`, so the pair is visible without asking. What the machine can count decides who may lead: **an OpenAI-led slice in any of those classes does not start in a repository whose check counts one reviewer identity** — Claude leads it and Codex reads it, and the gate means what it says. Where the check counts a read from both stacks, such a slice may start, read by the other side's badge. A Codex read of OpenAI's own work never satisfies that list, because no such pull request is opened. Everything outside the list is open to either lead, with the same cold read. **The badge answered on 21 September 2026**, which is what changes this: findings on one commit, then a clean verdict on the next that opened this repository's gate with no hand — `check.sh` reporting *the gate counts 2 reviewer(s) by 2 route(s)*. The lift reaches exactly as far as that: **only this repository has the badge installed**, so for every product the restriction stands word for word until the badge is installed there and that repository's own check counts it. A rule lifted on machinery nobody has watched work is the failure this section keeps paying for, and this one lifts no further than two run ids reach.
