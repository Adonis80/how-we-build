...
**The backup, and why the machine can count it.** It was ruled *main* at midday on 17 September, while the primary was out of credit, and put back behind the primary the same evening once a banked reset was spent. The machinery below was built for the first ruling and is unchanged by the second, which moved only when it may read. Write the ask on a pull request — the two words `/fable` and `review`, as the whole first line, so a sentence *about* the ask cannot spend one (the mistake made in the open on #28, where a note saying "no further asks" made one four lines below saying it). `.github/workflows/review.yml` then reads the diff with `claude-fable-5-1` at `--effort max` and every tool off, and posts its verdict as `github-actions[bot]`. Three things make it a gate rather than a way round, and none is decoration:
