...
- The reviewers, until further notice: **Claude Sonnet 5** and **Claude Fable 5.1**, both at maximum effort, and Codex is not one of them (his ruling, 22 September 2026, replacing his word of 18 September that named Sonnet 5 alone beside it). A reviewer is named in one place per repository — here, `review-gate.py` — so a change is one line, and the workflow that runs it is held against that line by the check. Each reads every pull request cold at that effort, from the committed diff and the pages the diff does not touch, before the pull request's own account, and signs a check run under the `juku-reviewer` App that no branch can wear and no writer can erase.

  **Today that file names one of them.** `REVIEWER_MODEL` is Sonnet 5; Fable 5.1 has no constant, no second check-run name and no workflow, in this repository or any other, and `GATE_REVIEWER` is still Codex. So this ruling is written down and not yet built, exactly as *the badge* was before it signed, and the same caveat governs it: **until a run shows a second reviewer answering in a repository, that repository's gate keeps the arrangement it has** — Codex plus one badge — and the classes below keep the rule they had. What lifts it is a run, not a sentence. Nothing here is a permission to merge on a red check. What is never the answer is a sentence permitting a merge while the check is red: one check carries the word caps, the file lists and the secret scan too, so any such permission waives those with it. Powerful open-weight models join as reviewers next, by the same route (his ruling, 18 September 2026).
