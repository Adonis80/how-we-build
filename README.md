...
- The reviewer, until further notice: Claude Sonnet 5 at maximum effort (his ruling, 18 September 2026, replacing that day's earlier word for Claude Fable 5.1). It is named here and nowhere else, so a change is one line. It reads every pull request cold, as a subagent of its own at that effort, from the committed diff and the tests before the pull request's own account, and its read goes on the pull request whole, naming the commit it read. Codex also reads wherever it is available, and is always asked on the classes in *Cold, and cross-vendor where it counts*. A finding from either holds the head: whoever left it, and whatever was said before or since, a head carrying one is never merged, because a finding outranks every other answer about that commit and only the push that answers it clears it. When the named reviewer is the one out, nothing is held up: Codex's clean read turns the check green by the ordinary route. The other way round is still owed — a repository's check counts only Codex today, so when Codex cannot read, the slice parks as it always has. Letting it move needs the check to count a read by the named reviewer that no branch can fake, and that is the work, not a sentence permitting a merge while the check is red: one check carries the word caps, the file lists and the secret scan too, so any such permission waives those with it. This repository's gate shows how first. Powerful open-weight models join as reviewers next, by the same route.
