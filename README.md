...
**The build board (his ruling, 25 September 2026).** One page, in plain English, that shows where every product is and how much is left, at `roadmap.juku.pro`: *"one place that I can go that explains everything"*. It is written here first, as the OpenRouter ruling was, because a ruling of his about how we build is not on the record until it is in this repository; the board itself is built by the pull request that carries this line and is not described here until it has run. Its source is each product's own `roadmap.json` — the `title`, `plain` and `status` of every item — and nothing else, so a product keeps the board current by keeping its roadmap current, which it already owes.
