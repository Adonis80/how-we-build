...
- The reviewer, until further notice: Claude Sonnet 5 at maximum effort (his ruling, 18 September 2026, replacing that day's earlier word for Claude Fable 5.1). It is named in one place per repository — here, `review-gate.py`'s `REVIEWER_MODEL` — so a change is one line, and the workflow that runs it is held against that line by the check. It reads every pull request cold at that effort, from the committed diff and the pages the diff does not touch, before the pull request's own account. Codex also reads wherever it is available, and is always asked on the classes in *Cold, and cross-vendor where it counts*. When the named reviewer is the one out, nothing is held up: Codex's clean read turns the check green by the ordinary route. The other way round is what *the badge* below is for, and until a run shows it answering, a repository's check still counts only Codex — so when Codex cannot read, the slice parks as it always has. What is never the answer is a sentence permitting a merge while the check is red: one check carries the word caps, the file lists and the secret scan too, so any such permission waives those with it. Powerful open-weight models join as reviewers next, by the same route (his ruling, 18 September 2026). **The models are named once, as roles** (his ruling, 22 September 2026: *“at any given day a new model release would require us to switch the main coder, the reviewer, and specialists like frontend”*). Every page, gate, workflow and product names a **role** — `reviewer-main`, `reviewer-second`, `coder-fast`, `coder-max`, `frontend-max`, `agent-fast` — and one registry in this repository says which model and provider fills each, and when that was last reviewed. **GLM fills `reviewer-main`** and the route is OpenRouter, behind a provider-neutral router so that no product ever depends on OpenRouter itself (his rulings, 22 September 2026); Kimi is the strength he names for front-end work. No version number is written in prose anywhere, because the version is the part that changes. Until the registry and a runner that can reach a non-Anthropic model exist, `reviewer-main` is filled by the model named at the top of this bullet — which is what *until further notice* means there.

- **The switch test, and it fails today.** Changing the model behind a role is **one edit to the registry and nothing else**: no workflow, no gate file, no page, no product repository, and no review of the review machinery. `check.sh` fails on a model identifier found anywhere outside the registry, which is what turns this from an intention into a property. Measured on 22 September 2026, swapping the reviewer is not one edit but seven: `REVIEWER_MODEL`; `REVIEWER_EFFORT`, which an open-weight model may not have; `REVIEWER_ASK`, spelt `/claude review`; the register key `claude`; `DEFAULT_REVIEWER`, also spelt for a vendor; the Anthropic CLI the reviewer workflow installs in order to run the read at all; and the Anthropic credential that CLI rides on. Six of the seven are gate files, so every model swap would drag a full gate review behind it. A check run named after its model would be the eighth, and is the reason the badge signs `juku-reviewer / <role>` and never `juku-reviewer / <model>`. Ending those seven is what the registry slice is for. A design for it exists and is the CTO's to settle: the consultant's brief *Open-weight model registry and routing*, 18 September 2026, found on 22 September in his Downloads folder, unread by any session, and carried in full on the pull request that queues it. It is built in a session attached to this repository, being schema, tests and a workflow.

```mermaid
flowchart LR
  A["A session, a gate,<br/>a product, a page"] -->|asks for a ROLE| R
  R["model-registry<br/>the one place a model is named"] -->|resolves to| P
  P["provider-neutral router"] --> O["OpenRouter"]
  P --> D["direct provider"]
  O --> M["the model filling that role today"]
  D --> M
  R -.->|"reviewer-main &rarr; GLM<br/>frontend-max &rarr; Kimi<br/>coder-fast &rarr; &hellip;"| R
```

**The drawing is the requirement.** Nothing on the left of the registry knows a model's name, so a swap is one edit inside the registry box and nothing outside it moves. Anything that names a model outside that box is the thing the switch test fails on. It is written here, and not carried in anyone's head, because the first telling of it reached no repository: on 22 September a search of all three repositories — every file and the whole history — returned no mention of OpenRouter or GLM, so every session since had started without it and the products spent the week on one reviewer. A ruling of his about how we build is not on the record until it is in this repository, exactly as a product decision is not until it is in that product's.
