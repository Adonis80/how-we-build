...
- The reviewer, until further notice: Claude Sonnet 5 at maximum effort (his ruling, 18 September 2026, replacing that day's earlier word for Claude Fable 5.1). It is named here and nowhere else, so a change is one line. It reads every pull request cold, as a subagent of its own at that effort, from the committed diff and the tests before the pull request's own account, and its read goes on the pull request whole, naming the commit it read. Codex also reads wherever it is available, and is always asked on the classes in *Cold, and cross-vendor where it counts*. Nothing waits on the other vendor: when Codex has said on the pull request that it cannot read, or has not read the head by the next session's start, the lead may merge on the named reviewer's clean read of that head with the check red, saying so in the merge message, as Hemz OS #61 did, and never on its own word. That exception ends in each repository, this one included, on the day its check can count a read by the named reviewer that no branch can fake; this repository's gate shows how first. Powerful open-weight models join as reviewers next, by the same route.
