...
**When the reviewer is unavailable** — allowance spent, the service down, or an ask that simply goes unanswered — the gate does not open and is not waived, and the slice parks exactly as it always did. What changes is what the wait is spent on (the Chairman's ruling, 14 September 2026): a session of the lead's own stack reads the change in its place — today a Fable 5.1 session, Claude leading — cold and in its own context, given the diff and these pages rather than the pull request's own account. Its review is posted whole, signed as the stand-in's, from the account the lead holds: the only one it has, which is the whole of why the gate does not count it. One comment on the pull request names the blocker, and no further *asks* — those are the thing that deepens the hole, because they spend an allowance that is already empty.
...
**Why the stand-in's read does not open the gate, since the ruling could be read as saying it should.** A verdict typed by the proposer's own account is the proposer clearing its own addition, however well shaped, and `AGENTS.md` forbids exactly that. It was built the other way once and taken out again: [#26](https://github.com/Adonis80/how-we-build/pull/26) gave the gate a shape it would count, and six cold reads found ten ways through it, each fix opening the next door. So the read happens and the gate keeps waiting for the one identity it can count — the identity the paragraph above is already waiting for.
