...
**When the reviewer is unavailable** — allowance spent, the service down, or an ask that simply goes unanswered — the gate does not open and is not waived, and the slice parks exactly as it always did. What changes is what the wait is spent on (the Chairman's ruling, 14 September 2026): a Fable 5.1 session reads the change in its place, cold and in its own context, given the diff and these pages rather than the pull request's own account, and its review is posted whole under its own name. One comment on the pull request names the blocker, and no further *asks* — those are the thing that deepens the hole, because they spend an allowance that is already empty.

**The stand-in reads again after every push, and that is the point of it being a stand-in.** A read clears only the commit it read, so answering the stand-in's findings makes a commit nobody has read — which would leave the head in exactly the state the gate exists to refuse. A stand-in read is a session of this stack's own, not the shared allowance, so it reads again — bound by the same two rounds that bind every review here, after which the remaining dissent stands or the slice shrinks, exactly as it would with Codex. Codex's ask is the scarce thing and the only one that waits: it is asked once when it returns, against a head that has already been read and corrected. That is the whole of what this buys — the same wait, spent on a change that is in better shape when the reviewer finally sees it.

**It does not make a cross-vendor slice startable.** On the classes *Cold, and cross-vendor where it counts* names above — and on that list alone, since a second copy of it here would be a second answer to the same question — the required read is the *other vendor's*, and a Fable stand-in is the same vendor as a Claude lead. *A slice that cannot get that read is not started* stands unchanged on the operating page and means what it says: those slices do not begin while Codex is away. The stand-in is what a parked slice does with its wait, never a reason to start one. Nothing watches for the reviewer's return, on any clock: a watch cannot see it, since only an ask can tell and a parked slice makes none. The return is found by the next session to start, whose ask on the oldest parked pull request either lands or is refused. Parked work never stops the line: the next slice starts from `main` in its own session — but `main` still names the parked slice as the top roadmap item, so a session starting on *build* alone would rebuild it. When the reviewer returns, parked pull requests re-enter review oldest first, one ask each. None of this reaches the Chairman: a parked slice is the CTO's wait, not his problem, and he hears of it only when it moves something he was promised.

**Why the stand-in's read does not open the gate, since the ruling could be read as saying it should.** It was built that way and then taken out again: [#26](https://github.com/Adonis80/how-we-build/pull/26) gave the gate a shape it would count as a stand-in verdict, and six cold reads found ten ways through it — a heading that carried the name and said the opposite, two verdict lines in one comment, no ask, an ask that satisfied itself, an ask posted afterwards, a finding erased by an edit. Each fix was two lines and each one opened the next door, because the thing being checked is typed by the account the check exists to bind. `AGENTS.md` says the proposer does not clear its own addition, and a verdict the proposer types is that, however well shaped. So the read happens and the gate keeps waiting for the one identity it can count. **What would change it: a reviewer identity for this stack on GitHub** — the same thing *Cold, and cross-vendor where it counts* is waiting for. With one, the stand-in stops being the proposer's own comment and every one of those ten findings goes away at once.
