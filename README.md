...
- **It is posted by GitHub Actions, not by a person.** A Fable read taken inside a builder session posts under the Chairman's own GitHub account, so its verdict cannot be told from the lead writing "looks fine" about its own work — the one thing this gate exists to refuse. No personal token can post under that name; only a workflow can. The review is real either way; what a session cannot do is *sign* it.
...
- A check that goes green in a pull request only when a reviewer has read the current commit, counting each reviewer by its own login and its own shapes: for the primary a submitted review or a plain comment naming the commit, for the backup the verdict line posted by `github-actions[bot]`, counted only behind the primary's own refusal and only while that is the primary's latest word there, and never allowed to clear a change to the gate or to anything under `.github/`. The reviewer that posts those verdicts is not built yet in any repository; the gate is ready for it.
