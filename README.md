...
**A product copies five files, and the only tap left is a fresh key.** `review-gate.py` holds the register, the door in the gate and the wiring checks; `.github/workflows/` gains `review.yml`, the `wake.yml` it calls and the `door.yml` that proves the door; `check.sh` runs the lot, and the product's own check workflow — `verify.yml` in Hemz OS, `check.yml` in Myst — gains the triggers the wake needs. **Installing the App on a repository is the session's own hands:** `PUT /user/installations/{installation_id}/repositories/{repository_id}` with the Mac's key does it in one call, answered `204` for Hemz OS on 21 September 2026. That call adds a repository to an installation and can do nothing else: an App's permissions are fixed on the App, and widening them is its owner's own approval. This line used to say the install was his part; a session read that, handed him the job, and he sent it back — the page was believed where one call would have tried it, which is *Numbers* broken on a claim about ourselves. **Read from the installation's own page on 22 September 2026**, past the identity code below: the App is installed on all three repositories, with read access to code and metadata and read-and-write access to checks and pull requests — it cannot write code anywhere. That dates `review-gate.py`'s header line, which still says the App is installed on this repository alone; it is a gate file, so the next code session on this repository corrects it, not a session changing words. **The one tap, and what it bought.** A private key cannot be read back out of the environment holding it, so the products could not be given the one this repository signs with: a fresh key has to be generated in the App's own settings, and GitHub emails him an identity code before those settings open. He gave that code on 22 September 2026; the key was generated, sealed into the `reviewer` environment of Hemz OS and of Myst, and deleted from disk, none of it passing through him. Each product's environment now holds the App's id and key behind a policy admitting `main` alone. This repository proved the route first, being public and holding no secrets, prices or customer data: the badge answered end to end here on 21 September 2026 — findings on `7125f34` and clean on `c8c0265`, both commits of [#43](https://github.com/Adonis80/how-we-build/pull/43), the clean one opening the gate with no hand. What each product still needs is the five files, and Myst a reading token of its own.
