...
**When Codex will not answer** — allowance spent, the service down, or an ask that simply goes unanswered — the slice no longer parks (the Chairman's ruling, 14 September 2026, which replaced the parking rule that stood until then). A Fable 5.1 session reads it instead, cold and in its own context, given the diff and these pages and not the pull request's own account; its review is posted whole, under its own name, naming the commit it read, in the shape `review-gate.py` documents and counts — which is also the one place that shape is written down. Codex is the better read, and nothing here hurries it: the gate counts the stand-in only while Codex has said nothing about that commit, and whatever Codex says settles it — findings, a clean pass, or a review that has finished with its verdict still coming. Within that silence a stand-in's finding keeps the head red, and the answer to it is a push, as ever. **Whether Codex must be asked before a stand-in may speak is not written here yet, on purpose.** A rule this page states and `check.sh` cannot hold is the hazard `AGENTS.md` names by its own name, so that rule and the check that holds it land together, in the pull request after this one.

**What the stand-in costs, said plainly rather than discovered later.** On the classes where the other vendor is required, a stand-in read is the same vendor as the lead, so the blind spot that cross-vendor reading exists to catch is not caught. It is a read that happens instead of a wait, not a read of equal strength, and the pull request says which one it got. And the machine cannot tell who wrote it: the review is posted by the account the CTO holds, not by an identity of its own, so `check.sh` enforces the shape, the commit and which answer outranks which, and trusts the session for the authorship. That is the weakest joint in this gate, and it closes on the day the paragraph above is waiting for. Parking goes with the ruling: nothing now waits on the reviewer's return, on any clock, and no pull request sits unread while it is away.
