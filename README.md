...
**A product copies four files, and no tap of his.** `review-gate.py` holds the register, the door in the gate and the wiring checks; `.github/workflows/` holds the reviewer, the wake it calls and the door's proof; `check.sh` runs the lot. **Installing the App on a repository is the session's own hands:** `PUT /user/installations/{installation_id}/repositories/{repository_id}` with the Mac's key does it in one call, answered `204` for Hemz OS on 21 September 2026 and confirmed on that repository's own *Installed GitHub Apps* page. This line used to say the install was his part; a session read that, handed him the job, and he sent it back — the page was believed where one call would have tried it, which is *Numbers* broken on a claim about ourselves. The repository's `reviewer` environment, its `main`-only policy and its App id go the same way, by API. What GitHub does keep for him is the identity code it emails before its own App settings will open, so generating a private key waits on that one code and nothing else does. This repository proved the route first, being public and holding no secrets, prices or customer data: the badge answered end to end here on 21 September 2026, clean on [#43](https://github.com/Adonis80/how-we-build/pull/43) and with findings on [#47](https://github.com/Adonis80/how-we-build/pull/47), which is both of its verdicts seen. The products were fitted the same day — the App installed on Hemz OS and Myst, each with a `reviewer` environment admitting `main` alone and the App's id and key sealed inside it — and that install stays in place: nothing in either repository invokes the App until its four files land. What each product still needs is those files and its own reading key.
