...
4. The `juku-reviewer` App installed on the repository, and its `reviewer` environment sealed with the App's key and the reviewer's — a session's hands, never his tap. That is the whole setup.
...
- The reviewers, until further notice: **Claude Sonnet 5** and **Claude Fable 5.1**, both at maximum effort, and Codex is not one of them (his ruling, 22 September 2026, replacing his word of 18 September that named Sonnet 5 alone beside it) — **the rule, and not yet the machine: what a repository counts today is still Codex plus one badge, set out at the foot of this bullet.** The backup, when one of them is out, is the open-weight reviewer his ruling of 18 September said would join next: **the route is OpenRouter and the first of them is GLM 5.2, as the main backup** (his ruling, on the record 22 September 2026). Those two rulings came the same day and fit together — the backup is what now stands where Codex stood. A reviewer is named in one place per repository — here, `review-gate.py` — so a change is one line, and the workflow that runs it is held against that line by the check. Each reads every pull request cold at that effort, from the committed diff and the pages the diff does not touch, before the pull request's own account, and signs a check run under the `juku-reviewer` App that no branch can wear and no writer can erase. What is never the answer is a sentence permitting a merge while the check is red: one check carries the word caps, the file lists and the secret scan too, so any such permission waives those with it.
  ...
  **And today that file names one of them.** `REVIEWER_MODEL` is Sonnet 5; Fable 5.1 and GLM 5.2 have no constant, no check-run name and no workflow, in this repository or any other, and `GATE_REVIEWER` is still Codex. So all three rulings above are written down and not yet built, exactly as *the badge* was before it signed, and the same caveat governs them: **until a run shows a second reviewer answering in a repository, that repository's gate keeps the arrangement it has** — Codex plus one badge — and the classes below keep the rule they had. What lifts it is a run, not a sentence.
