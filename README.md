...
**Cold, and cross-vendor where it counts.** Independent means a session of its own, forming its view from the diff and the tests before reading the pull request's own account. The same vendor is acceptable; the other vendor is preferred; and on pricing logic, live database mutation or schema, authentication and authorisation, public trust-boundary changes, deploy and release machinery, and this review gate itself, the other vendor is required. If it is unclear whether a change crosses a trust boundary, it does. Every pull request carries `Lead stack:` and `Reviewed by:`, so the pair is visible without asking. Until 21 September 2026 the machine counted one reviewer identity: `check.sh` turned green on a read by the Codex bot and nothing else. **An OpenAI-led slice in any of those classes does not start in a repository whose check counts one identity** — Claude leads it and Codex reads it, and the gate means what it says. A Codex read of OpenAI's own work never satisfies that list, because no such pull request is opened. Everything outside the list is open to either lead, with the same cold read. What would change this is *the badge* below, and it is now half proved rather than half built: the identity exists, this repository's gate counts it, and it has signed verdicts here — findings on `7125f34` and clean on `c8c0265`, both commits of [#43](https://github.com/Adonis80/how-we-build/pull/43), the clean one opening the gate with no hand. What has never been watched is the other direction: a findings verdict taking an already-green check back to red, which is the half the lift would rest on. The restriction therefore stands exactly as written until that runs — [#45](https://github.com/Adonis80/how-we-build/pull/45) is the pull request that would lift it and is parked on precisely that evidence — and a rule lifted on machinery nobody has watched work is the failure this whole section keeps paying for.
...
**A product copies five files, and the only tap left is a fresh key.** `review-gate.py` holds the register, the door in the gate and the wiring checks; `.github/workflows/` gains `review.yml`, the `wake.yml` it calls and the `door.yml` that proves the door; `check.sh` runs the lot, and the product's own check workflow — `verify.yml` in Hemz OS, `check.yml` in Myst — gains the triggers the wake needs. **Installing the App on a repository is the session's own hands:** `PUT /user/installations/{installation_id}/repositories/{repository_id}` with the Mac's key does it in one call, answered `204` for Hemz OS on 21 September 2026 and confirmed on that repository's own *Installed GitHub Apps* page. That also dates `review-gate.py`'s header line, which still says the App is installed on this repository alone; it is a gate file, so the next code session on this repository corrects it, not a session changing words. The installation's own repository list cannot be read from here either way — GitHub puts that page behind an identity code it emails him. That call adds a repository to an installation and can do nothing else: an App's permissions are fixed on the App, and widening them is its owner's own approval, so the call cannot hand a repository more than the App already had. This line used to say the install was his part; a session read that, handed him the job, and he sent it back — the page was believed where one call would have tried it, which is *Numbers* broken on a claim about ourselves. The `reviewer` environment and its `main`-only policy go the same way, by API: both products have one, re-checked on 22 September 2026, `main` alone and nothing else. **What is not fitted, checked rather than assumed on 22 September 2026:** neither product's `reviewer` environment holds the App's id or its key. The environment secrets list comes back empty for Hemz OS and for Myst, where this repository's holds both. A sealed secret cannot be read back out, so the key here cannot be copied across: a fresh private key has to be generated in the App's own settings, and that is the one thing GitHub keeps for him — it emails an identity code before those settings will open. Until that is done the badge cannot answer in either product, whatever else has landed there, and saying otherwise is the failure [#34](https://github.com/Adonis80/how-we-build/pull/34) closed on. This repository proved the route first, being public and holding no secrets, prices or customer data: the badge answered end to end here on 21 September 2026 — findings on `7125f34` and clean on `c8c0265`, both commits of [#43](https://github.com/Adonis80/how-we-build/pull/43), which is both of its verdicts seen, and the clean one opened the gate with no hand. What each product still needs is those five files, a key sealed in its `reviewer` environment, and its own reading token.
