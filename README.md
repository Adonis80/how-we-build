...
**Anthropic.** Building happens in a code session started attached to the repository. A Cowork or chat session advises, and may change words through the Mac by pull request. The hard stop is enforced by the product's `.claude/settings.json`. **Its hands are the Claude Chrome Extension**, in a Cowork session — a console, a dashboard, a setting no pipeline can reach. A code session has no browser: it puts the task in CI, and asks the advisory session for the rest. Neither is a second lead; advising is not owning.
