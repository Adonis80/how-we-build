...
**If the badge cannot answer.** Requiring both reviewers on a gate change means a reviewer that cannot sign holds every gate change shut. Most of the ways that happens need no pull request to fix — a rotated key, a suspended App, an uninstalled one all live in settings, outside the tree. The one that bites is a merged commit to `review.yml` or `review-gate.py` that stops the badge answering: the repair is itself a gate change, and it waits for the reviewer it would repair. **Nothing is built for this and nothing should be** — §13 asks for the same failure twice before machinery, and this has happened none, while the machinery it asks for is a single-vendor route into the most dangerous diff in the repository. The route that already exists is the one to use: the Chairman merges the revert on his own ruling, recorded on the pull request. That is not his hand becoming the control — it is the exception this rulebook already grants, used once, in the open. Raised as a P1 by Codex on [#46](https://github.com/Adonis80/how-we-build/pull/46), where the commit that made the badge load-bearing also carried the bug that would have caused it.
