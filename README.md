...
**If the badge cannot answer.** Requiring every reviewer on a gate change means a reviewer that cannot sign freezes gate changes. Most ways that happens need no pull request at all — a rotated key, a suspended App, an uninstalled one all live in settings, outside the tree. The one that bites is a merged commit to `review.yml` or `review-gate.py` that stops the badge answering, where the repair is itself a gate change and waits for the reviewer it would repair. **This rulebook offers no route out of that, and must not**: the paragraph above forbids a sentence permitting a merge while the check is red, because that one check carries the word caps, the file lists and the secret scan too, and CHARTER §12 forbids a lead disabling, weakening or skipping a required check at all. A recovery written here would waive all of it to rescue one reviewer. **What makes it bearable is the bound**: only *gate* changes freeze, because everything else clears on one reviewer and Codex is unaffected — and the same rule that creates the dependency is what makes a badge-breaking merge unlikely, since two vendors must now both clear the commit that would do it. Recorded as an accepted risk rather than a hole awaiting a patch. Raised as a P1 by Codex on [#46](https://github.com/Adonis80/how-we-build/pull/46) — twice: once for the gap, and again when the recovery I first wrote turned out to be the forbidden sentence.
