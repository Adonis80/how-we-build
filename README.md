...
Step 5 of the loop — an independent reviewer reads every pull request cold — has two readers, and the Chairman's ruling of 17 September 2026 sets the order: *"we better use GPT sol 5.6 as primary reviewer because Fable will hit a wall. and if thats not available then default for Fable 5.1. this is because although it Claude, its still different to Opus 5 which is the default builder for claude code mode."* So **Codex is the primary and reads first**, and **Fable 5.1 at max effort is the backup**, standing in only where Codex has said on that same pull request that it cannot. The second half of his sentence is why the backup's read counts as independent at all: Fable is a different model from Opus 5, which is what builds. (*"sol 5.6"* names no setting anyone can hold: the GitHub reviewer offers no model choice, so **the primary** means the Codex reviewer, whichever model OpenAI runs behind it.) Either one reads the *change*: the diff, the tests, what regressed, and whether the pull request's claims match its code. Neither challenges a design; that is the consultant's lane below. Both take their brief from the repository's own `AGENTS.md`, both post on the pull request where the CTO answers them, and one gate reads them both. Nothing is pasted between models, and nothing passes through the Chairman.

Codex is reached by writing `@codex review` on a pull request once code review is switched on for the repository; it then reads the current commit and posts its findings there. It is one reviewer for every repository under this rulebook, this one included — which is why this repository, though not a product, carries an `AGENTS.md` of its own. Which repositories it can reach is decided by the GitHub app's repository access, set once for all of them; no other setup is needed.

**The backup, and what the gate is ready to count.** It was ruled *main* at midday on 17 September, while the primary was out of credit, and put back behind the primary the same evening once a banked reset was spent. **The gate's half is built and proved here; the reviewer that would post the verdicts it counts is not installed yet.** That comes in its own pull request, with the credential, once it can be exercised end to end — the reason is under *What is not here yet* below. Everything in this section is what the gate will and will not accept, and it is true today whether or not anything is posting.

A backup verdict counts only where all four hold, and none is decoration:

- **It is posted under the reviewer's own login, and that login does not exist yet.** A Fable read taken inside a builder session posts under the Chairman's own GitHub account, so its verdict cannot be told from the lead writing "looks fine" about its own work — the one thing this gate exists to refuse. The review is real either way; what a session cannot do is *sign* it. The gate counted `github-actions[bot]` until the backup's own first cold read said what that name is: the login of **every** workflow token in this repository, not an identity belonging to a reviewer. Anyone who can push — which every lead session can — may put a workflow on a throwaway branch, never opened as a pull request and so never seen by the guard below, and have it post the verdict line on any pull request here. So the gate counts one login belonging to the reviewer alone, as the primary has its own, and **until that identity exists it counts no backup at all**: the verdict shape, posted by Actions or by anybody else, reads as no read. The identity is the Chairman's one tap and it is asked for by the pull request that installs the reviewer, not this one.
- **The workflow that posts it is read from the default branch.** `issue_comment` runs a workflow file from the default branch, never from the pull request's head. On `pull_request` a branch could edit its own reviewer — or the brief it is handed — and write itself a clean pass.
- **It may not clear the gate itself.** A pull request touching `review-gate.py`, `check.sh` or anything under `.github/` waits for the primary however long that takes. The breadth is kept for the reason that survives: the machinery that decides is machinery only the other vendor may clear. It used to be argued as the thing that made forgery *impossible* — to post a verdict you must add a workflow, and adding one is what makes you ineligible — and the backup's own read showed that false, because the workflow need not be in the pull request under review at all. What shuts that door is the bullet above, a login no pushed branch can wear; this one is not load-bearing against a forger and is no longer described as though it were. The rulebook already required the other vendor on the gate; that was a sentence, and a sentence is not a door. Without this, a gate change would be the *easiest* thing to slip past: ask the primary, let it refuse, and the refusal is precisely what makes the backup eligible. **And what this bullet is not: enforcement.** The copy of the rule that `check.sh` runs is the *proposed* tree's, so a branch changing the gate can loosen it and pass its own check — true of every line in `check.sh` today, with a backup or without one. Calling the branch's own copy "defence in depth" was a claim this repository could not cash, and it is withdrawn here rather than left standing. The check that decides has to run from the default branch over a tree it only reads; that is the **wake-job repair**'s work, queued next. Until it lands, this bullet binds an honest reviewer and not a dishonest branch, and whoever installs the workflow makes it refuse such a pull request before posting anything.
- **Only behind the primary's refusal — its latest word when the backup wrote, and written about this very commit.** One allowance serves every repository and it resets: on 17 September the primary refused on #30 at 11:46 and read that same pull request at 15:54, a banked reset having been spent between. A gate counting the morning's refusal would hand the afternoon to the backup while the primary sat available — the opposite of the ruling. And "latest word" is read across both halves of the page: the primary *edits* its summary comment when a review finishes, and its findings arrive as a submitted review rather than a comment at all, so a list of comments in creation order can show a refusal as the last word long after it has answered. Two further bindings came from the backup's own read. A refusal names no commit, so it counts only if it was written after this head reached the pull request — taken from GitHub's clock, never from the commit's own date, which whoever pushes sets — or else one refusal on Monday's head hands the backup every push after it and the primary is never asked about any of them. And each verdict is judged as at the moment it was written, so a refusal landing afterwards cannot reach back and make legitimate a read taken while the primary could still answer. Read forwards, that same rule is why a real read's findings stay red when the primary later reads the same commit clean: the answer to a finding is a push, whoever found it.

**And it does not wake the check.** A comment posted with a workflow's token starts no workflow run, and the wake job in `check.yml` matches the primary's login alone — so a backup verdict lands and the check has to be re-run for it. *A review clears only the commit it read*, below, promises green by itself with no hand re-run: that promise is the primary's, and making it true for the backup belongs to the **wake-job repair** queued next. Said here rather than left for the reviewer's own pull request to inherit.

With nothing posting a verdict, the gate stays red, which is the correct state and not a fault: a gate that opens when its reviewer is missing is not a gate. `check.sh` holds all of it on every run — 25 fakes refused, and the gate goes red under each of fifteen loosenings, every one of them tried: a prefix match instead of the exact verdict line, any author instead of the reviewer's own login, the shared Actions name counted again, the gate guard removed, the commit no longer named, the short form no longer resolved, the backup counted with no refusal behind it, a refusal still counted after the primary has answered since, a refusal counted that was written before this head existed, an unknown arrival read as a refusal standing, eligibility judged when the check runs rather than when the backup wrote, an edited verdict, clean pass or refusal counted, and the gate's answer ranked above the primary's own clean pass.

**What is not here yet, and why it was taken out.** The reviewer itself — the workflow, its brief, and the rule deciding whether it may read a given pull request — was built alongside this and removed from it. Five rounds of cold review found five ways round that workflow, every one in its shell rather than in the gate's decisions, which is the part no selftest can reach without a credential to run it against. The fifth arrived after the CTO had undertaken to split rather than patch again, so it was split. The gate's decisions are held by the selftest and land here; the workflow lands when there is a credential to exercise it end to end, and its pull request carries what the five rounds established.

**Cold, and cross-vendor where it counts.** Independent means a session of its own, forming its view from the diff and the tests before reading the pull request's own account. The same vendor is acceptable; the other vendor is preferred; and on pricing logic, live database mutation or schema, authentication and authorisation, public trust-boundary changes, deploy and release machinery, and this review gate itself, the other vendor is required. If it is unclear whether a change crosses a trust boundary, it does. Every pull request carries `Lead stack:` and `Reviewed by:`, so the pair is visible without asking. The machine counts **one** reviewer identity today and is ready to count a second the day that reviewer's own login exists: `BACKUP` is empty, so every backup verdict reads as no read at all. What the gate is prepared to do is read each by its own login and its own shapes, and count the backup only while the primary cannot answer. Of the classes above the machine holds exactly one — the gate's own files, by path. Pricing, live database mutation or schema, authentication, trust boundaries and deploys are prose here and the CTO keeps them; a product that wants them held names its own. **An OpenAI-led slice in any of those classes still does not start** — Claude leads it and Codex reads it, and the gate means what it says. A Codex read of OpenAI's own work never satisfies that list, because no such pull request is opened. Everything outside the list is open to either lead, with the same cold read. The Claude reviewer this paragraph used to wait for arrived on 17 September 2026, and it changed one sentence and not the rest: being the same vendor as the lead, it stands in when the primary cannot answer, and clears everything except the list — the gate included.

**A review clears only the commit it read.** A later push voids it: a round's fixes land as one push and the CTO asks once, and a repository's check goes green only when the reviewer has read the current commit — by itself, with no hand re-run, whichever shape the answer takes: findings arrive as a submitted review, a clean pass as a plain comment naming the commit, and both count as a read (fixed 9 September, after a clean pass left a pull request red overnight). This repository goes further, because here the proposer may not clear its own addition: its gate tells the two shapes apart and opens on a clean read alone, so a head the reviewer left findings on stays red until the push that answers them. A report to the Chairman names the commit that was reviewed.
...
**When no reviewer can be had** — the primary's allowance spent or its service down, and the backup without a credential or unable to answer — the gate does not open and is not waived. With a backup that draws on no shared allowance this should now be rare, and what follows is what happens when it is not. The slice parks: one comment on the pull request names the blocker, then no further pushes and no further asks, which only deepen the hole. Nothing watches for the reviewer's return, on any clock: a watch cannot see it, since only an ask can tell and a parked slice makes none. The return is found by the next session to start, whose ask on the oldest parked pull request either lands or is refused. Parked work never stops the line: the next slice starts from `main` in its own session — but `main` still names the parked slice as the top roadmap item, so a session starting on *build* alone would rebuild it. When the reviewer returns, parked pull requests re-enter review oldest first, one ask each. None of this reaches the Chairman: a parked slice is the CTO's wait, not his problem, and he hears of it only when it moves something he was promised.
...
Each reviewer takes its brief from the repository's own `AGENTS.md` — Codex because that is where its tool looks, the backup because its workflow sends that file, read from the default branch. The brief lives here, once. A pointer alone was tried on two real reviews and the marks of the brief vanished from them — no addressee, no account of what was checked, no confidence — while a review with the brief in front of it carried all three. So a product's `AGENTS.md` ends with a short `## Review guidelines` section: the pointer here, the least of the brief the tool needs in front of it, and the hazards particular to that product. Those few lines are a copy, kept only because the tool cannot follow a link, and they change only when this brief does.
...
- A check that goes green in a pull request only when a reviewer has read the current commit, counting each reviewer by its own login and its own shapes — the whole of it in *The independent reviewer* above.
