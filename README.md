...
4. Both reviewers reachable: code review switched on for the repository in Codex — the Chairman's tap — and `.github/workflows/review.yml` with the `CLAUDE_CODE_OAUTH_TOKEN` secret behind it, for the reviewer *The independent reviewer* names. That is the whole setup.
...
Step 5 of the loop — an independent reviewer reads every pull request cold — is done by a session of its own on GitHub, before the Chairman sees anything. It reviews the *change*: the diff, the tests, what regressed, and whether the pull request's claims match its code. It does not challenge a design; that is the consultant's lane below. Its findings land on the pull request, addressed to the CTO, who answers them there. Nothing is pasted between models, and nothing passes through the Chairman. It takes its brief from the repository's own `AGENTS.md` — which is why this repository, though not a product, carries one.

**Who reviews is one named setting** (the Chairman's ruling, 17 September 2026: *Sonnet 5 at maximum effort is the default reviewer for every product*). He said in the same breath that he may switch it again, so the identity is not spread across these pages: it is `DEFAULT_REVIEWER` at the top of `review-gate.py`, and `GATE_REVIEWER` on the line below it is the other vendor that the classes below require. Every rule here is written by role, so changing either is one line and nothing else moves. The logins those two names resolve to are in `REVIEWERS`, beside them. Asking is `/claude review` for Sonnet 5 and `@codex review` for Codex, written on the pull request. Which repositories Codex can reach is decided by the GitHub app's repository access, set once for all of them; Sonnet 5 runs from each repository's own `.github/workflows/review.yml` and needs the credential named below.

**Cold, and cross-vendor where it counts.** Independent means a session of its own, forming its view from the diff and the tests before reading the pull request's own account. The default reviewer reads everything; and on pricing logic, live database mutation or schema, authentication and authorisation, public trust-boundary changes, deploy and release machinery, and this review gate itself, **the other vendor is required and reads it instead** — money, sign-in, live customer data and release machinery, in his words of 17 September. While Claude leads, that means Codex. If it is unclear whether a change crosses a trust boundary, it does. Every pull request carries `Lead stack:` and `Reviewed by:`, so the pair is visible without asking. Two things follow and neither is softened. **An OpenAI-led slice in any of those classes still does not start**, because a Codex read of OpenAI's own work is not cross-vendor and no such pull request is opened; Claude leads it and Codex reads it. And **the default reviewer being the same vendor as the lead is not a way round that list** — it reads what is outside it, at the same cold standard, and nothing on it.

**In this repository the list has one door, and it is machine-held.** Of those classes only the last can arise here, this being the rulebook rather than a product: a pull request touching `check.sh`, `review-gate.py` or anything under `.github/workflows/` opens on Codex's clean read alone, and a clean read by the default reviewer leaves it red saying so. That rule used to hold by accident — Codex was the only identity the gate could count, so nothing else could clear anything. Counting a second reviewer spends the accident, so the rule is written into `review-gate.py` as a door. The whole of `.github/workflows/` is on the list rather than the two files in it today, because a read by the default reviewer is a comment from GitHub Actions, and any workflow a branch adds could post one about its own head.

**The credential this needs is one, and it is his.** Sonnet 5 reads through the unmodified Claude Code binary, which is what Anthropic's own terms sanction for a subscription credential — not a hand-written call to the Messages API, whatever header the token rides on. So each repository needs the secret **`CLAUDE_CODE_OAUTH_TOKEN`**, made by `claude setup-token`: it rides his existing plan, so there is no new spending and no API account. The token lasts a year, so the day it was made is the day it must be renewed by, and that date belongs in the repository that holds it rather than in anyone's memory. His part is one tap — signing in and approving on his own Claude account; a Cowork session makes the token and seals it into the repository's secrets through GitHub's API, where no person and no model sees it (*The two stacks*). **Until it exists in a given repository the reviewer there fails closed**: the workflow runs, finds no credential, says so on the pull request, and the gate stays red — which is correct, not a fault — so Codex reads that repository's pull requests meanwhile, as it did before.
...
**The reviewer's budget is finite and shared.** One allowance serves every repository under this rulebook: spent on one product, it is spent for all, and a words-only pull request draws on the same pool. That is why the ask comes once per round, with the round's fixes batched into one push — an ask per small push reads the same code many times over and empties the allowance. The 17 September ruling is the largest relief that pool has had: with Sonnet 5 reading everything outside the required classes, Codex is asked only where the other vendor is required, and its allowance stops being the thing every slice queues behind. Learned the expensive way on 9 September: nine asks on nine small pushes to one Juku Perfume pull request ended reviews by mid-afternoon, and within minutes the same wall refused Hemz OS on two pull requests — the same failure, twice, in two products, which is what clears the charter's gate for this rule.

**When the reviewer a change requires is unavailable** — allowance spent, the credential missing, or the service down — the gate does not open and is not waived, and the other reviewer is not substituted for it. A change in the required classes waits for the other vendor however long it takes; everything else waits for the default reviewer. The slice parks: one comment on the pull request names the blocker, then no further pushes and no further asks, which only deepen the hole. Nothing watches for the reviewer's return, on any clock: a watch cannot see it, since only an ask can tell and a parked slice makes none. The return is found by the next session to start, whose ask on the oldest parked pull request either lands or is refused. Parked work never stops the line: the next slice starts from `main` in its own session — but `main` still names the parked slice as the top roadmap item, so a session starting on *build* alone would rebuild it. When the reviewer returns, parked pull requests re-enter review oldest first, one ask each. None of this reaches the Chairman: a parked slice is the CTO's wait, not his problem, and he hears of it only when it moves something he was promised.
...
- A check that goes green in a pull request only when the reviewer has read the current commit, counting both shapes of answer — a submitted review, and a plain comment naming the commit — and waking by itself on either, with no hand re-run.
- The reviewer named in one setting, not spread through the words, and the classes where the other vendor is required held as a door rather than a sentence: a change to the repository's own review machinery opens on that vendor's read alone. `review-gate.py` here is the worked example.
- `.github/workflows/review.yml`, run only from `issue_comment` so a branch cannot edit its own reviewer, asking only on a collaborator's word, and failing closed with no credential.
...
By pull request only; `main` is protected. `check.sh` runs in CI and refuses a page over 600 words, a file not on the list, anything that looks like a secret, a reviewer the check would never wake for (or a wake for one the gate would not count), a reviewer workflow that could be run from a branch's own head, and — in a pull request — a commit no reviewer has read clean: unread, read and left findings on, or, where the change is to the review machinery, read clean by the wrong vendor. Adding a rule, step, file, check or agent needs the charter's gate (§13): the same failure twice, in two separate product tasks. Removing one, or correcting wording, needs no gate; nor does a change the Chairman rules himself. **The CTO settles and merges changes to this page and this repository, without the Chairman** (his ruling, 9 September 2026: "you are the CTO — don't ask me about such things in future"). He is asked for money, a permission, a product outcome or a picture, and nothing else. The reviewer still reads every change cold, and the gate above still applies. Every product picks the change up at its next session, because every session reads this repository live. There is no copy anywhere to refresh.
