...
- `PRODUCT.md` carries a section with five headings: the destination, the whole and its parts · the ladder, and the rung we are on · the loop: what the AI commits to, and what reality answers · what is captured, what deliberately is not, and under which notice · the proof: measure, baseline, repeatability, current figure or "unknown". `RICH-DATA.md` says how to fill them. "Unknown" and "deliberately not captured" are valid answers.
