...
4. The `juku-reviewer` App installed on the repository — a session's own hands, by API, never his tap. Nothing signs there yet: the route that reviews a product's pull requests from here is not built (*What every product carries*).
...
**A product copies four files, not a paragraph.** `review-gate.py` holds the register and the wiring checks; `.github/workflows/` holds the reviewer, the wake it calls and the door's proof; `check.sh` runs the lot. Installing the App is a session's hands (*How a product joins*). This repository proved the route first, being public and holding no secrets, prices or customer data; how the products follow, and what of that is not built, is in *What every product carries*.
