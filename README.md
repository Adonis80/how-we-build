...
Step 5 of the loop — an independent reviewer reads every pull request cold — has two readers, and the Chairman's ruling of 17 September 2026 says which one leads: *"Astra is out of credit for this week. We have updated the rule for you to use Claude Fable 5.1 at Max effort as the main reviewer."* So **Fable at max effort is the main reviewer**, asked on every pull request, and **Codex is the other vendor**, asked where the list below requires one. Either one reads the *change*: the diff, the tests, what regressed, and whether the pull request's claims match its code. Neither challenges a design; that is the consultant's lane below. Both take their brief from the repository's own `AGENTS.md`, both post on the pull request where the CTO answers them, and one gate reads them both. Nothing is pasted between models, and nothing passes through the Chairman.
...
**The main reviewer, and why the machine can count it.** Write the ask on a pull request — the two words `/fable` and `review`, as the whole first line, so a sentence *about* the ask cannot spend one (the mistake made in the open on #28, where a note saying "no further asks" made one four lines below saying it). `.github/workflows/review.yml` then reads the diff with `claude-fable-5-1` at `--effort max` and every tool off, and posts its verdict as `github-actions[bot]`. Three things make it a gate rather than a way round, and none is decoration:
...
- A check that goes green in a pull request only when a reviewer has read the current commit, counting each reviewer by its own login and its own shapes: for the other vendor a submitted review or a plain comment naming the commit, for the main reviewer the verdict line its workflow writes. And the main reviewer itself — the workflow that posts as `github-actions[bot]`, triggered by a comment so GitHub runs it from the default branch, and a gate that refuses to let it clear a change to itself, to the gate, or to anything else under `.github/`.
