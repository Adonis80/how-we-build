...
Step 5 of the loop — an independent reviewer reads every pull request cold — is done by the reviewer named in *What every product carries*, and by nobody else. **The Chairman's ruling, 22 September 2026: "Do not use Codex as a reviewer. Only use Sonnet 5 and Fable 5.1 as the main reviewer."** So Codex is not asked, on any repository under this rulebook, and `@codex review` is not written on a pull request again. The reviewer reads the *change*: the diff, the tests, what regressed, and whether the pull request's claims match its code. It does not challenge a design; that is the consultant's lane below. It signs as *the badge* below — a check run only the App can create — and the CTO answers its findings on the pull request. Nothing is pasted between models, and nothing passes through the Chairman.

**What that ruling costs, said plainly.** Today only this repository's gate counts the badge; the product repositories' gates still count the Codex bot and nothing else, so until the badge reaches them **no product slice can pass a gate at all**. That is the whole of the work it creates, and it is the CTO's, not his. The route that fits the other rulings: the reviewer runs **from this repository**, where the environment door is real because the repository is public, and signs its verdict onto the product's pull request through the App, which is installed on Hemz OS and Myst. No App key then sits in a private repository, where the free plan ignores the door — and his ruling of 21 September, that money waits for revenue, stays intact. `review-gate.py` and the workflows are code, so that lands from a session attached to this repository, not from these words.

**Cold, and one stack (his ruling, 22 September 2026).** Independent means a session of its own, forming its view from the diff and the tests before reading the pull request's own account — not a different vendor. The classes that used to demand the other vendor — pricing logic, live database mutation or schema, authentication and authorisation, public trust-boundary changes, deploy and release machinery, and this review gate itself — now demand the badge's own read, signed as a check run nobody with write access can forge, edit or erase, and the proposer still may not clear its own change here. If it is unclear whether a change crosses a trust boundary, it does. Every pull request still carries `Lead stack:` and `Reviewed by:`, so who read it is visible without asking. What is given up with Codex is a second vendor's eyes, and it is given up knowingly on his word; what replaces it is a reader that cannot be forged and a gate that stays shut without it.
...
- The reviewer: **Claude Sonnet 5 at maximum effort**, with **Fable 5.1** when it cannot run (his rulings, 18 and 22 September 2026). It is named in one place per repository — here, `review-gate.py`'s `REVIEWER_MODEL` — so a change is one line, and the workflow that runs it is held against that line by the check. It reads every pull request cold at that effort, from the committed diff and the pages the diff does not touch, before the pull request's own account. **Codex is not asked at all** (his ruling, 22 September 2026), so a repository whose gate still counts only the Codex bot cannot pass a gate until the badge reaches it — the route is in *The independent reviewer* above. What is never the answer is a sentence permitting a merge while the check is red: one check carries the word caps, the file lists and the secret scan too, so any such permission waives those with it. Powerful open-weight models join as reviewers next, by the same route (his ruling, 18 September 2026).
