...
**The backup, and why the machine can count it.** It was ruled *main* at midday on 17 September, while the primary was out of credit, and put back behind the primary the same evening once a banked reset was spent. The machinery below was built for the first ruling and is unchanged by the second, which moved only when it may read. Write the ask on a pull request — the two words `/fable` and `review`, as the whole first line, so a sentence *about* the ask cannot spend one (the mistake made in the open on #28, where a note saying "no further asks" made one four lines below saying it). `.github/workflows/review.yml` then reads the diff with `claude-fable-5-1` at `--effort max`, and posts its verdict as `github-actions[bot]`. It runs **in the default-branch checkout with `Read`, `Glob` and `Grep` and nothing else** — it has to reach the pages its brief tells it to judge against, so what bounds it is not *no tools* but *only tools that read*: `--restricted` takes away the ones that run commands or code, `check.sh` parses the tool list and fails on a fourth, `--permission-prompts none` denies anything that would ask, and the checkout is made with `persist-credentials: false` so the job's token is never in `.git/config` for a `Read` to find. What it sees is the state *before* the change; the proposed tree reaches it only as a diff. Four things make it a gate rather than a way round, and none is decoration:

- **It posts as Actions, not as a person.** A Fable read taken inside a builder session posts under the Chairman's own GitHub account, so its verdict cannot be told from the lead writing "looks fine" about its own work — the one thing this gate exists to refuse. No personal token can post under that name; only a workflow can. The review is real either way; what a session cannot do is *sign* it.
- **`issue_comment`, never `pull_request`.** That trigger runs the workflow file from the **default branch**, so a branch cannot rewrite its own reviewer — nor the brief it is handed, which is read from the default branch for the same reason.
- **It may not clear the gate itself, and it is turned away before it reads one.** A pull request touching `review-gate.py`, `check.sh` or anything under `.github/` waits for Codex however long that takes. The breadth is what keeps the guard above true: to forge a verdict you must post as Actions, to post as Actions you must add a workflow, and adding one is exactly what makes this reviewer ineligible. The rulebook already required the other vendor on the gate; that was a sentence, and a sentence is not a door. Without it a gate change would be the *easiest* thing to slip past: ask the primary, let it refuse, and the refusal is precisely what makes the backup eligible. **Where this is decided matters as much as the rule.** `check.sh` runs the *proposed* tree's copy of the gate, so a branch changing it could loosen the guard, delete the case that holds it, and pass its own check. So the question is asked first by the workflow, from the default branch, before the backup reads anything — a tree the branch cannot touch. The gate asks again when the check runs; that second answer is defence in depth, never the enforcement.
- **Only behind the primary's refusal, and only while that is its latest word there.** One allowance serves every repository and it resets: on 17 September the primary refused on #30 at 11:46 and read the same pull request at 15:54, a banked reset having been spent between. A gate counting the morning's refusal would hand the afternoon to the backup while the primary sat available — the opposite of the ruling. So anything the primary has said since a refusal makes it available again, and it is asked. The workflow does not decide this for itself: it asks `review-gate.py --eligible`, the same file the check runs, so the reviewer and the gate that counts it cannot drift apart.

It fails closed — no credential, no review, and the gate stays red — and it says on the pull request that it did not read, in a shape the gate cannot mistake for one that did. `check.sh` holds the whole of it on every run: 25 fakes refused, and the gate goes red when the rule is loosened six ways — a prefix match instead of the exact verdict line, any author instead of Actions, the gate guard removed, the commit no longer named, the backup counted with no refusal behind it, or a refusal still counted after the primary has answered since.

**Cold, and cross-vendor where it counts.** Independent means a session of its own, forming its view from the diff and the tests before reading the pull request's own account. The same vendor is acceptable; the other vendor is preferred; and on pricing logic, live database mutation or schema, authentication and authorisation, public trust-boundary changes, deploy and release machinery, and this review gate itself, the other vendor is required. If it is unclear whether a change crosses a trust boundary, it does. Every pull request carries `Lead stack:` and `Reviewed by:`, so the pair is visible without asking. The machine counts two reviewer identities, and they are not interchangeable: the gate reads each by its own login and its own shapes, the backup counts only while the primary cannot answer, and on the classes above only the primary's read clears at all. **An OpenAI-led slice in any of those classes still does not start** — Claude leads it and Codex reads it, and the gate means what it says. A Codex read of OpenAI's own work never satisfies that list, because no such pull request is opened. Everything outside the list is open to either lead, with the same cold read. The Claude reviewer this paragraph used to wait for arrived on 17 September 2026, and it changed one sentence and not the rest: being the same vendor as the lead, it stands in when the primary cannot answer, and clears everything except the list — the gate included.
...
- A check that goes green in a pull request only when a reviewer has read the current commit, counting each reviewer by its own login and its own shapes: for the primary a submitted review or a plain comment naming the commit, for the backup the verdict line its workflow writes. And the backup itself — the workflow that posts as `github-actions[bot]`, triggered by a comment so GitHub runs it from the default branch, counted only behind the primary's own refusal and only while that is the primary's latest word there, and never allowed to clear a change to the gate or to anything under `.github/`.
