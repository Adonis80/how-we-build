...
**The badge has answered.** It signed verdicts on 21 September 2026 on [#46](https://github.com/Adonis80/how-we-build/pull/46), and findings on [#56](https://github.com/Adonis80/how-we-build/pull/56) on 22 September — the sentence that stood here until then, that it *"has never signed a verdict"*, was already stale when two separate reads caught it. Only this repository's gate counts it; the product gates still count the Codex bot, and until the badge reaches them a product slice cannot pass a gate.
...
- The reviewer, until further notice: Claude Sonnet 5 at maximum effort (his ruling, 18 September 2026, replacing that day's earlier word for Claude Fable 5.1). It is named in one place per repository — here, `review-gate.py`'s `REVIEWER_MODEL` — so a change is one line, and the workflow that runs it is held against that line by the check. It reads every pull request cold at that effort, from the committed diff and the pages the diff does not touch, before the pull request's own account. **Codex is retired as a reviewer** (his ruling, 22 September 2026), so where the badge counts it is the only reviewer: no second read on any class, and nothing covering its absence — when it cannot read, the slice parks. **Built in this repository alone so far**, whose gate counts the badge and no longer counts Codex at all.
  ...
  **The retirement is sequenced, and that is the CTO's call rather than his.** A product's gate is that product's own `review-gate.py`, and both still count only the Codex bot. Read as an immediate blanket prohibition this would stop Hemz OS and Myst merging anything at all until the badge is ported — two products halted, which is a consequence of the ruling nobody asked for and the opposite of what every other rule here is for. So: **a product keeps the gate it has until the pull request that ports the badge lands in it, and that port is the next work.** No product is blocked in the meantime and no session parks on this. It is reversible in a sentence if he wants the harder reading, and the transition is already in front of him on [#51](https://github.com/Adonis80/how-we-build/pull/51), which put the same question to him from the other side.
