...
- The reviewers, until further notice: **Claude Sonnet 5** and **Claude Fable 5.1**, both at maximum effort, and Codex is not one of them (his ruling, 22 September 2026, replacing his word of 18 September that named Sonnet 5 alone beside it). They are named in one place per repository — here, `review-gate.py` — so a change is one line each, and the workflows that run them are held against those lines by the check. Each reads every pull request cold at that effort, from the committed diff and the pages the diff does not touch, before the pull request's own account, and signs a check run under the `juku-reviewer` App that no branch can wear and no writer can erase. One clean read opens an ordinary change; the classes in *Cold, and read twice where it counts* need both, and one reviewer out holds up only those. **A repository whose badge has never signed counts no reviewer at all and can merge nothing**, so the App install and the sealed `reviewer` environment are what carry this ruling to a product — a session's hands, listed in *How a product joins*. What is never the answer is a sentence permitting a merge while the check is red: one check carries the word caps, the file lists and the secret scan too, so any such permission waives those with it. Powerful open-weight models join as reviewers next, by the same route (his ruling, 18 September 2026).
