...
4. The `juku-reviewer` App installed on the repository, its `reviewer` environment sealed with the App's key and the reviewer's, the five files that read them — `review-gate.py` and the four workflows — and, **until those files have landed, Codex review switched on as well**. That last is not a leftover: adding the five files is itself a change to the gate, and `verdict()` opens a gate change on `GATE_REVIEWER` alone, so a repository that skips it **cannot complete its own setup** — the change that gives it a badge is the one change its badge may not clear. Codex comes off when the gate stops requiring it and not before, which is the order the ruling above reaches a product in. A session's hands throughout, never his tap. As of 22 September 2026 the App and the environments are done on all three repositories; the files are what is outstanding. That is the whole setup.
...
Step 5 of the loop — an independent reviewer reads every pull request cold — is done, until further notice, by the two reviewers named in *What every product carries*, in every repository under this rulebook. **Codex does not review** (the Chairman's ruling, 22 September 2026) — **which is the rule, and not yet the machine.** It builds and nothing more. Where this page still shows Codex reading, and where `check.sh` on a change to `check.sh`, `review-gate.py` or a workflow still tells a session in as many words to write `@codex review`, that is what the machinery does until the ruling is built. **And those two words in a pull request comment are the ask itself**, wherever in the comment they fall — so a session explaining this transition spends a read on the very reviewer it is explaining the retirement of. That happened on this page's own pull request, in a sentence warning about it. Until the ruling is built, name that ask without writing it — and the same holds for `/claude review` here, for the same reason, until this repository's trigger is narrowed the way Myst's was. It is described here rather than written out, because a session may stop reading once it believes it knows enough, and the paragraph that would have corrected it is four paragraphs down. They review the *change*: the diff, the tests, what regressed, and whether the pull request's claims match its code. They do not challenge a design; that is the consultant's lane below. `/claude review` in a comment makes a reviewer read the current commit and sign a verdict on it, which the CTO answers on that pull request. **Here that is anywhere in the comment**, because `review.yml` matches on `contains`; Myst's copy was narrowed to the head of a comment on 21 September, after a sentence about an ask spent a read. This one has not been narrowed, so the warning below applies to both asks in this repository. Nothing is pasted between models, and nothing passes through the Chairman. The same two serve every repository under this rulebook, this one included — which is why this repository, though not a product, carries an `AGENTS.md` of its own. What they can reach is the `juku-reviewer` App's repository access, plus each repository holding the `reviewer` environment that keys them.

**Cold, and read twice where it counts — which is the rule, and is not what runs today.** Independent means a session of its own, forming its view from the diff and the tests before reading the pull request's own account. Two reviewers read every pull request — different models, each cold, each signing a verdict of its own — **and one of the two is wired, so a reviewer reads once and that is what clears a commit.** On pricing logic, live database mutation or schema, authentication and authorisation, public trust-boundary changes, deploy and release machinery, and this review gate itself, **both** must read it clean before the gate opens — **again the rule: today one clean read still clears every one of them.** Everywhere else one clean read is enough and always was. If it is unclear whether a change crosses a trust boundary, it does.

**And of those six, exactly one is visible to a machine.** `touches_the_gate` classifies a change to the gate by file pattern. The other five — pricing, live database mutation or schema, authentication and authorisation, public trust boundaries, deploy and release machinery — match no pattern at all, before this ruling or after it: they are the CTO remembering to ask, and nothing enforces them. That was true of the wording this replaces too, and it is written here because a reader who takes **both** for a guarantee would stop asking on the five where asking is the only thing there is.

**Why the one the machine can see opens the way it does.** `review-gate.py` opens a gate change on `GATE_REVIEWER` alone, deliberately, so that findings from a reviewer with no standing on it cannot bolt a door it may not open (round #38, accepted in part). "Both must be clean" is the opposite of that, so turning the rule on is a change to the clearing logic and a selftest case, not a sentence here. Every pull request carries `Lead stack:` and `Reviewed by:`, so the pair is visible without asking.

**What that ruling gave up, written here rather than glossed.** Until 22 September this list required *the other vendor*, so that no vendor marked its own homework: Codex read Claude's work. It does not read any more, and on a Claude-led slice the two reviewers are now the same vendor as the lead — two models standing in for two companies. On an OpenAI-led slice the old guarantee survives untouched, because Codex builds and Claude reads. That is the whole of the cost and it is stated so nobody has to rediscover it; what has to be real in exchange is two reads a branch cannot forge, which is what *the badge* below is. **Until a repository's badge has signed a verdict its check counts no reviewer at all**, so the ruling reaches a repository only when its App is installed and its `reviewer` environment sealed — a rule lifted on machinery nobody has watched work is the failure this section keeps paying for.

**A review clears only the commit it read.** A later push voids it: a round's fixes land as one push and the CTO asks once, and a repository's check goes green only when the reviewer has read the current commit — by itself, with no hand re-run, whichever shape the answer takes. A verdict is one check run, whose commit and conclusion are fields GitHub fills rather than sentences a gate must parse, so there is no shape to miss — which is the whole reason a verdict is signed rather than written. The prose shapes are still in `review-gate.py`, still matched exactly, and still doing the work, because the reviewer that writes them is still what clears a change to the gate's own files: its findings arrive as a submitted review and its clean pass as a plain comment naming the commit, which are **the two shapes** the rest of this paragraph and the carries list both lean on. The night a clean pass left a pull request red for want of one is why they are matched exactly rather than loosely. They go when the ruling above is built, not before. This repository goes further, because here the proposer may not clear its own addition: its gate tells the two shapes apart and opens on a clean read alone, so a head the reviewer left findings on stays red until the push that answers them. A report to the Chairman names the commit that was reviewed.
...
**When the gate can count no read** — a reviewer out, its allowance spent or the service down — the gate does not open and is not waived. One reviewer out holds up only the classes above that need both; everywhere else the other's clean read opens the gate by the ordinary route. The slice parks: one comment on the pull request names the blocker, then no further pushes and no further asks, which only deepen the hole. Nothing watches for the reviewer's return, on any clock: a watch cannot see it, since only an ask can tell and a parked slice makes none. The return is found by the next session to start, whose ask on the oldest parked pull request either lands or is refused. Parked work never stops the line: the next slice starts from `main` in its own session — but `main` still names the parked slice as the top roadmap item, so a session starting on *build* alone would rebuild it. When the reviewer returns, parked pull requests re-enter review oldest first, one ask each. None of this reaches the Chairman: a parked slice is the CTO's wait, not his problem, and he hears of it only when it moves something he was promised.
...
- The reviewers, until further notice: **Claude Sonnet 5** and **Claude Fable 5.1**, both at maximum effort, and Codex is not one of them (his ruling, 22 September 2026, replacing his word of 18 September that named Sonnet 5 alone beside it) — **the rule, and not yet the machine: what a repository counts today is still Codex plus one badge, set out at the foot of this bullet.** The backup, when one of them is out, is the open-weight reviewer his ruling of 18 September said would join next: **the route is OpenRouter and the first of them is GLM 5.2, as the main backup** (his ruling, on the record 22 September 2026). Those two rulings came the same day and fit together — the backup is what now stands where Codex stood. A reviewer is named in one place per repository — here, `review-gate.py` — so a change is one line, and the workflow that runs it is held against that line by the check. Each reads every pull request cold at that effort, from the committed diff and the pages the diff does not touch, before the pull request's own account, and signs a check run under the `juku-reviewer` App that no branch can wear and no writer can erase. What is never the answer is a sentence permitting a merge while the check is red: one check carries the word caps, the file lists and the secret scan too, so any such permission waives those with it.

  **A ruling is not on the record until it is in this repository.** The OpenRouter route was given once before and reached no file: on 22 September a search of all three repositories — every file and the whole history — returned no mention of OpenRouter or GLM, so every session since had started without it and the products spent the week on one reviewer. It is written here, and not carried in anyone's head, exactly as a product decision is not on the record until it is in that product's repository.

  **And today that file names one of them.** `REVIEWER_MODEL` is Sonnet 5; Fable 5.1 and GLM 5.2 have no constant, no check-run name and no workflow, in this repository or any other, and `GATE_REVIEWER` is still Codex. So all three rulings above are written down and not yet built, exactly as *the badge* was before it signed, and the same caveat governs them: **until a run shows a second reviewer answering in a repository, that repository's gate keeps the arrangement it has** — here, Codex plus one badge — and the classes above keep the rule they had. What lifts it is a run, not a sentence.

  **And what a product's gate counts today is Codex** — checked in the code rather than inferred from the ruling. The `juku-reviewer` App is installed on all three repositories and each `reviewer` environment has been sealed with its id and key (established 22 September 2026), so the key stopped being the blocker that morning; what neither product has is the five files — `review-gate.py` and the four workflows — without which no badge can sign there, and until they land each product's own gate counts the one identity it was written for. So a product pull request today still needs the ask its gate requires, **and a session that reads only the ruling above would park instead of making it.** Here, where the files exist, `verdict()` opens an ordinary change on either reviewer and a gate change on `GATE_REVIEWER` alone. That asymmetry is also the bootstrap in *How a product joins*: the change that gives a product a badge is a gate change, so only Codex can let it in.
