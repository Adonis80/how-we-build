...
**Anthropic.** Building happens in a code session started attached to the repository. A Cowork or chat session advises, and may change words through the Mac by pull request. The hard stop is enforced by the product's `.claude/settings.json`. **Its hands are Claude's own, used from a Cowork session: the Chrome extension, computer use on the Mac, and the Claude app's built-in browser** — for a console, a dashboard, a setting no pipeline can reach (his rulings: 14 September 2026, *you should do all manual tasks with chrome extension*; 17 September 2026, *make sure that in future all manual tasks are done through Claude chrome extension and or computer use*). A code session has none of them. It puts the job in CI; where CI cannot do it, it writes the job on its pull request under **Hands needed:** with every choice already made, and tells him only where to take it: a Cowork session in the product's Project, and the word *hands*. That session reads the open pull requests for the line and does the work. Neither is a second lead; advising is not owning.

**His part is only a tap no hand may make.** The hands may not sign in for him, type a password or a secret into a web page, get past an are-you-human check, pay, or approve a sign-in key on his account by themselves. A session takes such a job to that one tap and hands him the tap alone — the link, what he will see, the words to reply — never the job, and never a choice of how. Where each tool reaches: the extension cannot open Claude's own pages at `claude.ai`, so those go to the built-in browser, after asking him once. A secret goes from where it is made into the repository's secrets through GitHub's API, sealed with the repository's public key: never typed into a web page, never shown in a chat. Learned 17 September 2026. A Hemz OS code session handed him *add one credential — an Anthropic API key or a Claude subscription token*, a job with a choice attached. The Cowork session that took it over chose the subscription token, since it needs no new spending, made it, and stored it sealed. Its own safety guard refused to approve the key without him, which is right. Claude's approval page then stalled twice in his Chrome and went through at once in the built-in browser. All he did was sign in and tap *Authorize*.
...
**The session sets them; the Chairman is handed no paste (his ruling, 14 September 2026).** It works in the Claude app's built-in browser, signed in to his Claude account: `claude.ai/projects` → *New project* → the name and one line saying what it is → *Create project* → *Instructions* → *Edit instructions* → the template, whole → *Save instructions*. An existing Project is the same route from its own page. Nothing is ever added to a Project's *Context*: it holds instructions and no files. The session then reloads the page, reopens the instructions, reads them back and says what it set — a Project is not reported as done on the strength of having typed into it. Until 17 September 2026 this ran in his Chrome through the extension; that day the extension refused to open `claude.ai` at all, and the built-in browser did the job, read back after a reload. It asks him once before using that browser, and needs the desktop app open and signed in to Claude; only when it is not is he handed the text whole, and that is the exception, not the route.
