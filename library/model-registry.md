...
**Who reads what.** Decision 0008 reverses his 22 September pairing, which put the open-weight model behind Sonnet as the main backup: the cheaper model now reads first. An ordinary change goes to `reviewer-main`. If it does not answer with a verdict, `reviewer-fallback` reads it. If neither answers, the commit stays unread and the gate stays red. A change in a risky class, or with any file whose kind the list does not know, goes to `reviewer-risky`, read whole at max, with nothing behind it. `review-gate.py` refuses a registry that sets those efforts otherwise, and runs the fallback on its own cases. The reviewers read the registry from `main`, so a pull request never picks its own reviewer. None of this has run live yet: the first read through the registry is cited on [#103](https://github.com/Adonis80/how-we-build/issues/103).
