...
Scope: every session, in either stack. Open when: deciding whether to stay in a session or start fresh, and before leaving one.

At every turn, decide whether the next turn stays here or starts fresh (his ruling). Stay only while all three hold:

- **Same work:** the same slice or bounded question continues.
- **Cheaper to stay:** the useful unresolved context here costs less than rebuilding it from the boot list.
...
**Before leaving**, put every durable fact in GitHub, and make the pull request and roadmap handover sufficient on their own. If the opening words for the next session would have to carry project state, the handover is not finished. Advisory work with no repository yet gets one temporary `START-HERE.md`: the bounded question, what is agreed, the next action, the files that matter. It is superseded the moment the result lands in GitHub.

**Say so, as a claim that can be wrong.** A turn that ends "ready to start fresh session" also states that the next session has full context: the repository carries what it needs, and nothing has to be pasted. Stating it turns a sign-off into a claim, and a claim is checked before it is made.
