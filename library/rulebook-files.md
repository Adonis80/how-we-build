...
**And `model-registry/` and `consensuses/`.** The registry is the one place a model is named ([decision 0008](https://github.com/Adonis80/how-we-build/issues/103)): `library/model-registry.md`. `consensuses/` holds a consensus committed verbatim, one folder per project, when a build must read it from GitHub; it is read like the decision issues, never loaded into a working session.
