...
**And `model-registry/` and `consensuses/`.** The registry is the one place a model is named ([decision 0008](https://github.com/Adonis80/how-we-build/issues/103)): `library/model-registry.md`. `consensuses/` holds the paper a consensus was built on, committed verbatim, one folder per project, when a build must read it from GitHub, and — his ruling, 25 September 2026, first kept for decision 0009 — the frozen record a consensus ends on, the same words as its decision issue; where the agreement itself was reached is its decision issue, as decision 0008's amendment records for the 18 September paper. It is read like the decision issues, never loaded into a working session.
